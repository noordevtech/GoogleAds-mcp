...
```bash
python run_server.py        # preferred — adds src/ to sys.path
python -m src               # equivalent, via src/__main__.py
LOG_LEVEL=DEBUG python -m src
```

For HTTP exposure (port 8000), build the Docker image — `entrypoint.sh` wraps the stdio server with `mcp-proxy`. See **Deployment (Coolify)** below.
...
`pyproject.toml` declares `pytest` / `pytest-asyncio` as dev deps and the upstream README mentions `pytest tests/`, but **no `tests/` directory exists**. There is currently no test suite to run. Verification is via compile-check + AST extraction + helper smoke tests + live verification post-deploy (see **Testing approach** below).

### Config file search (file-based auth, alternative to env vars)

`auth.GoogleAdsAuthManager` searches three locations in order if env vars aren't set:

1. `~/.config/google-ads-mcp/config.json`
2. `~/.google-ads-mcp.json`
3. `./google-ads-config.json`

Env vars always override file values. See `config.example.json` for the schema.

### API version

Pinned to **Google Ads API v20**. `error_handler.RetryableGoogleAdsClient.get_service` defaults to `version="v20"`, and error documentation URLs are built against `/v20/errors`. When bumping, search for the literal `"v20"` and update both spots.
